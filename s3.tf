...
### s3 admin - artifact bucket
data "aws_iam_policy_document" "s3admin-artifact" {
  statement {
    effect    = "Allow"
    resources = ["arn:aws:s3:::${local.name}-artifact/*"]
    actions   = ["s3:*"]
  }
...
resource "aws_iam_policy" "s3admin-artifact" {
  name   = "${local.name}-s3admin-artifact"
  policy = "${data.aws_iam_policy_document.s3admin-artifact.json}"
}

resource "aws_s3_bucket" "storage-artifact" {
  bucket = "${local.name}-artifact"
  tags   = "${var.tags}"
}
